...
function two_elec_grad(mol::Molecule, basis::Basis,
  grad_flags::Union{Dict{String,Any},Dict{Any,Any}}; output)
  
  debug::Bool = scf_flags["debug"]
  load::String = scf_flags["load"]

  return two_elec_grad_kernel(mol,basis; output=output, debug=debug, load=load)
end
