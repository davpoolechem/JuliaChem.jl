...
long long int get_workmem_c(long long int derivative_order, 
  long long int max_ang_momentum) 
{
  long long int workmem_bytes = simint_ostei_workmem(derivative_order, 
    max_ang_momentum);
  
  return ceil(workmem_bytes/sizeof(double));
}
